...
export class StytchAuth {
	...
	static initialize(projectId: string, secretKey: string, publicToken: string, env: 'test' | 'live' = 'test') {
	...
	static getClient(): stytch.Client {
		if (!this.client) {
			throw new Error('Stytch client not initialized. Call StytchAuth.initialize() first.');
		}
		return this.client;
	}
	...
	static async sendMagicLink(email: string, redirectUrl: string, expirationMinutes: number = 30): Promise<any> {
		const client = this.getClient();
	...
	static getOAuthEndpointUrl(env: 'test' | 'live', endpoint: string): string {
		...
		const stytchBaseUrl = env === 'test' ? 'https://login-test.srv.im' : 'https://api.stytch.com';
	...
	static getAuthorizationServerMetadata(baseUrl: string) {
		...
		return {
			...
			scopes_supported: ['openid', 'profile', 'email', 'read:calendars', 'manage:calendars', 'read:events', 'calendars:*'],
...
export function hasRequiredScope(userScopes: string[], toolName: string): boolean {
...
export function validateToolAccess(userContext: UserContext | null, toolName: string): { authorized: boolean; error?: string } {
	if (!userContext) {
		return {
			authorized: false,
