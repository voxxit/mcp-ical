...
describe("Security Tests", () => {
  ...
  describe("Command Injection Prevention", () => {
    test("timezone detection should not use execSync", () => {
      const timezoneManager = new TimezoneDateManager();
  ...
  describe("SSRF Protection", () => {
    ...
    test("should reject cloud metadata endpoints", async () => {
      await expect(
        calendarManager.subscribeCalendar(
          "http://169.254.169.254/calendar.ics",
          "test-metadata",
        ),
      ).rejects.toThrow("Access to internal resources is not allowed");
    });
  ...
  describe("Resource Limits", () => {
    test("should reject dates with excessive range", async () => {
      ...
      await expect(calendarManager.getEvents(start, end)).rejects.toThrow(
        "Invalid date range",
      );
    });
    ...
    test("should limit search query length", async () => {
      ...
      await expect(calendarManager.searchEvents(longQuery)).rejects.toThrow(
  ...
  describe("Security Configuration", () => {
    ...
    test("should validate search queries correctly", () => {
      ...
      expect(securityConfig.isValidSearchQuery("team meeting tomorrow")).toBe(
        true,
      );
  ...
  describe("Error Sanitization", () => {
    test("should not expose internal paths in production mode", () => {
      ...
      try {
        ...
        const testError = new Error(
          "ENOENT: no such file or directory, open '/home/user/.secret'",
        );
        const sanitized = sanitizeMethod(testError, "test");

        expect(sanitized).not.toContain("/home/user/.secret");
        expect(sanitized).toBe(
          "An error occurred while processing calendar data",
        );
      } finally {
    ...
    test("should mask sensitive information in development mode", () => {
      ...
      try {
        ...
        const testError = new Error(
          "Failed to fetch https://secret-server.com/api with Bearer abc123token",
        );
        const sanitized = sanitizeMethod(testError, "test");
