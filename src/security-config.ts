...
export class SecurityConfigManager {
  ...
  private loadSecureDefaults(): SecurityConfig {
    return {
      maxCalendarSubscriptions: this.getEnvNumber('ICAL_MAX_CALENDARS', 10, 1, 50),
      ...
      maxCalendarNameLength: this.getEnvNumber('ICAL_MAX_NAME_LENGTH', 100, 1, 200),
      maxSearchQueryLength: this.getEnvNumber('ICAL_MAX_QUERY_LENGTH', 1000, 1, 2000),
      maxDateRangeDays: this.getEnvNumber('ICAL_MAX_DATE_RANGE_DAYS', 365, 1, 1095), // Max 3 years
      maxRRuleOccurrences: this.getEnvNumber('ICAL_MAX_RRULE_OCCURRENCES', 1000, 1, 5000),
      rruleTimeoutMs: this.getEnvNumber('ICAL_RRULE_TIMEOUT_MS', 5000, 1000, 30000),
      fetchTimeoutMs: this.getEnvNumber('ICAL_FETCH_TIMEOUT_MS', 30000, 5000, 120000),
      maxContentSizeBytes: this.getEnvNumber('ICAL_MAX_CONTENT_SIZE_MB', 10, 1, 100) * 1024 * 1024,
      production: process.env.NODE_ENV === 'production',
    };
  }

  private getEnvNumber(envVar: string, defaultValue: number, min: number, max: number): number {
    ...
    if (isNaN(parsed)) {
      console.warn(`Warning: Invalid value for ${envVar}: "${value}". Using default: ${defaultValue}`);
      return defaultValue;
    }

    if (parsed < min || parsed > max) {
      console.warn(`Warning: Value for ${envVar} (${parsed}) outside safe range [${min}, ${max}]. Using default: ${defaultValue}`);
      return defaultValue;
    }
  ...
  private validateEnvironment(): void {
    ...
    if (process.env.TZ) {
      try {
        ...
        console.warn(`Warning: Invalid timezone in TZ environment variable: ${process.env.TZ}`);
      }
    }

    // Security warnings for development
    if (!this.config.production) {
      console.warn('⚠️ Running in development mode - verbose error messages enabled');
    }
  }
