...
export class KVCalendarManager {
  ...
  private async loadSubscriptions(): Promise<
    Record<string, CalendarSubscription>
  > {
  ...
  private async saveSubscriptions(
    subscriptions: Record<string, CalendarSubscription>,
  ): Promise<void> {
  ...
  private async getCachedCalendar(name: string): Promise<CalendarCache | null> {
    ...
    if (
      memoryCached &&
      Date.now() - memoryCached.lastFetched.getTime() < this.MEMORY_CACHE_TTL
    ) {
      return memoryCached;
    }
   ...
   */
  private async setCachedCalendar(
    name: string,
    data: CalendarCache,
    ttlMinutes: number,
  ): Promise<void> {
    ...
    await this.env.AUTH_STORE.put(cacheKey, JSON.stringify(data), {
  ...
  private filterEventsByDateRange(
    ...
    return events.filter((event) => {
      if (start && Temporal.ZonedDateTime.compare(event.end, start) < 0)
        return false;
      if (end && Temporal.ZonedDateTime.compare(event.start, end) > 0)
        return false;
      return true;
    });
  }
