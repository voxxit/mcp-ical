 ...
 */
export const createMcpScopeMiddleware = (requiredScope: string) =>
  createMiddleware<{
    Bindings: Env;
  }>(async (c, next) => {
    const authHeader = c.req.header("Authorization");
...
async function validateStytchJWT(token: string, env: Env) {
  ...
  if (!jwks) {
    ...
    const stytchDomain = env.STYTCH_DOMAIN;
    jwks = createRemoteJWKSet(
      new URL(`${stytchDomain}/v1/sessions/jwks/${env.STYTCH_PROJECT_ID}`),
    );
  }
