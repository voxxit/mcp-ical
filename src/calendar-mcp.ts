...
export class CalendarMCP extends McpAgent<Env, unknown, AuthenticationContext> {
  ...
  async init() {
    ...
    if (!this.props || !this.props.claims || !this.props.claims.sub) {
      // Default to anonymous if no auth context
      this.props = {
        claims: {
          sub: "anonymous",
  ...
  get server() {
    ...
    server.resource(
      ...
      async (uri, params) => {
        ...
        const calendar = calendars.find(
          (cal) => cal.name === decodeURIComponent(name),
        );
    ...
    server.tool(
      ...
      async ({ url, name, refreshInterval = 60 }) => {
        await this.calendarService.subscribeToCalendar(
          url,
          name,
          refreshInterval,
        );
        return this.formatResponse(
          `Successfully subscribed to calendar "${name}"`,
        );
      },
    );
    ...
    server.tool(
      ...
      async ({ name }) => {
        ...
        return this.formatResponse(
          `Successfully unsubscribed from calendar "${name}"`,
        );
      },
    );
    ...
    server.tool(
      ...
      async ({ query, calendarName, startDate, endDate }) => {
        ...
        return this.formatResponse(`Found ${events.length} matching events`, {
          events,
        });
      },
    );
    ...
    server.tool(
      ...
      async ({ startDate, endDate, calendarName }) => {
        ...
        return this.formatResponse(`Retrieved ${events.length} events`, {
          events,
        });
      },
    );
    ...
    server.tool(
      ...
      async ({ date, calendarName }) => {
        ...
        return this.formatResponse(`Retrieved agenda for ${date}`, {
          date,
          events,
        });
      },
