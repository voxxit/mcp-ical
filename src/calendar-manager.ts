...
export class CalendarManager {
  ...
  private validateCalendarUrl(url: string): void {
    ...
    if (url.length > config.maxUrlLength) {
      throw new Error(`URL exceeds maximum length of ${config.maxUrlLength} characters`);
    }
  }
  ...
  private validateCalendarName(name: string): void {
    if (!this.securityConfig.isValidCalendarName(name)) {
      const config = this.securityConfig.getConfig();
      throw new Error(`Invalid calendar name. Must be 1-${config.maxCalendarNameLength} characters, alphanumeric with spaces, hyphens, underscores, dots only. No path traversal sequences allowed.`);
    }
  }
  ...
  private validateRefreshInterval(refreshInterval: number): void {
    if (!Number.isInteger(refreshInterval) || refreshInterval < 1 || refreshInterval > 10080) {
      throw new Error('Refresh interval must be between 1 and 10080 minutes (1 week)');
    }
  }
  ...
  private validateDateRange(startDate: Date, endDate: Date): void {
    if (!this.securityConfig.isValidDateRange(startDate, endDate)) {
      const config = this.securityConfig.getConfig();
      throw new Error(`Invalid date range. Must be valid dates with start before end, and range cannot exceed ${config.maxDateRangeDays} days.`);
    }
  }
  ...
  private validateSearchQuery(query: string): void {
    if (!this.securityConfig.isValidSearchQuery(query)) {
      const config = this.securityConfig.getConfig();
      throw new Error(`Invalid search query. Must be 1-${config.maxSearchQueryLength} characters and not just whitespace.`);
    }
  }
  ...
  async subscribeCalendar(url: string, name: string, refreshInterval: number = 60) {
    ...
    if (this.subscriptions.size >= config.maxCalendarSubscriptions) {
      throw new Error(`Maximum number of calendar subscriptions (${config.maxCalendarSubscriptions}) reached`);
    }
  ...
  async getEvents(
    ...
    for (const name of calendarsToCheck) {
      try {
        ...
        const sanitizedMessage = this.sanitizeErrorMessage(error, 'calendar-processing');
    ...
    try {
      ...
      for (const occurrence of limitedOccurrences) {
        ...
        if (Date.now() - startTime > MAX_PROCESSING_TIME) {
          console.warn(`RRULE processing timeout exceeded for event ${vevent.uid}, stopping expansion`);
          break;
        }
      ...
      const sanitizedMessage = this.sanitizeErrorMessage(error, 'rrule-expansion');
